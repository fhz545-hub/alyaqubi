...
import LegacyEmbed from "@/components/teachers/LegacyEmbed";
import { useAuth } from "@/contexts/AuthContext";
import { hasPermission } from "@/store/permissionsStore";

export default function TeacherAdminAffairsPage() {
      ...
      <div className="min-h-screen bg-background p-6" dir="rtl">
        <div className="max-w-2xl mx-auto">
          <Card className="p-8 text-center border-destructive/30">
            ...
            <p className="text-muted-foreground mt-2">الشؤون الإدارية تتطلب صلاحية «إدارة شؤون المعلمين».</p>
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      <div className="max-w-[1700px] mx-auto p-4 md:p-6 space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-primary/15 text-primary grid place-items-center"><ClipboardList className="w-6 h-6" /></div>
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-foreground">الشؤون الإدارية</h1>
              <p className="text-sm text-muted-foreground">واجهة مستقلة واسعة للإجراءات والنماذج الإدارية الخاصة بالمعلمين.</p>
            </div>
          </div>
          <Button variant="outline" asChild><Link to="/teacher-affairs"><ArrowRight className="ml-2 h-4 w-4" /> مركز شؤون المعلمين</Link></Button>
        </div>
        <LegacyEmbed src={`/legacy/admin-affairs.html${location.search || ""}`} source="admin_affairs" title="الشؤون الإدارية والمتابعة" />
      </div>
    </div>
  );
}