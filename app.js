...
function viewSettings(){
  ...
  $("#btnLoadSample", importCard).addEventListener("click", async ()=>{
    ...
    state.studentsCount = await db.countStudents();
    updateCounters();
    $("#importHint", importCard).textContent = `تم استيراد: ${inserted} طالب`;
    toast("تم الاستيراد");
  });
  ...
  $("#btnImport", importCard).addEventListener("click", async ()=>{
    ...
    state.studentsCount = await db.countStudents();
    updateCounters();
    $("#importHint", importCard).textContent = `تم استيراد: ${inserted} طالب (متجاهل: ${parsed.rows.length - students.length})`;
    toast("تم الاستيراد");
  });
...
function parseCSV(text){
  ...
  // Find header line within first 40 lines
  const maxScan = Math.min(40, linesAll.length);
  let headerIdx = 0;
  for(let i=0;i<maxScan;i++){
    const l = linesAll[i];
    const n = normalizeArabic(l);
    if(n.includes("اسم") && (n.includes("الطالب") || n.includes("طالب")) && (n.includes("رقم") || n.includes("id"))){
      headerIdx = i;
      break;
    }
  }
  ...
  const headers = splitCSVLine(headerLine, delim).map(x=>x.trim());
  ...
  for(let i=headerIdx+1;i<linesAll.length;i++){
    ...
    for(let c=0;c<headers.length;c++){
      obj[headers[c] || `col${c}`] = String(parts[c] ?? "").trim();
    }
    rows.push(obj);
  }
  return { rows, headers, delim, headerIdx };
}
...
function mapRowsToStudents(rows){
  ...
  for(const r of rows){
    ...
    const name = pick(r, ["اسم الطالب","الاسم","student_name","name"]);
    const studentNo = pick(r, ["رقم الطالب","student_no","id","studentId"]);
    const phone = pick(r, ["الجوال","جوال","phone","mobile"]);
    const className = pick(r, ["الفصل","class","classroom"]);
    const grade = pick(r, ["رقم الصف","الصف","grade"]);
