...
import { code39Canvas } from './assets/barcode.js';
...
function renderStudentCard(student){
  ...
  const cv = $('#bc');
  if(cv){
    try{ code39Canvas(cv, String(student.nid||'')); }catch{ /* ignore */ }
  }
}
...
function bindEvents(){
  document.addEventListener('click', async (e)=>{
    ...
    try{
      ...
      if(act==='edit-student'){
        const st = Store.state.students.find(s=>String(s.nid)===String(btn.dataset.nid));
        return openStudentForm(st||null);
      }
      if(act==='delete-student') return await deleteStudent(btn.dataset.nid);
